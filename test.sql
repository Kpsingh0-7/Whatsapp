...
CREATE TABLE food_orders (
    ...
    order_status_id INT,
    FOREIGN KEY (shop_id) REFERENCES food_shop(shop_id),
    FOREIGN KEY (guest_id) REFERENCES food_guest(guest_id)
);
...
SELECT 
    c.conversation_id,
    fs.shop_name,
    fg.name AS guest_name,
    m.content AS last_message,
    m.sent_at AS last_message_time,
    umc.shop_unread_count,
    umc.guest_unread_count
FROM conversations c
JOIN food_shop fs ON c.shop_id = fs.shop_id
JOIN food_guest fg ON c.guest_id = fg.guest_id
...
LEFT JOIN unread_message_counts umc ON umc.conversation_id = c.conversation_id
ORDER BY m.sent_at DESC;
